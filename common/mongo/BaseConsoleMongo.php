...
class BaseConsoleMongo extends BaseMongo{

    //默认表名
	public static $collection_name = 'default';
    public static $db_name = 'mongodb_console'; 
    ...
    public function __construct($action){
        ...
        $title = $this->getMethodName($calss_name, $action->actionMethod);
    ...
    public function begin($action = '', $title = '') {
        ...
        // 可以在这里添加数据库级别的全局属性
        // $this->addLogField("BaseConsoleMongo",'123456' );
    }
    ...
    public function end($result = '') {
        $can_log = false;
        foreach (static::$details as $key => $value) {
            if(count($value)>1){
                $can_log = true;
            }  
        }
